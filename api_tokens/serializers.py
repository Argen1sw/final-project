    ...
    * Returns the created Alert record.
    ...
    def validate_effect_radius(self, value):
        if value > 100000 or value < 0:  # 100 km = 100,000 meters
            raise serializers.ValidationError(
                "The radius of effect cannot exceed 100 km or be less than 0 m.")
        return value
    ...
    def create(self, validated_data):
        ...
        try:
            ...
            raise serializers.ValidationError(
                {"geocoding": f"Geocoding failed: {str(e)}"})
        ...
        if hazard_type:
            ...
            if not model_class:
                raise serializers.ValidationError(
                    {"hazard_type": "Invalid hazard type."})
            try:
                ...
                raise serializers.ValidationError(
                    {"hazard_data": f"Error creating Hazard: {str(e)}"})
